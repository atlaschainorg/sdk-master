...
pub mod syscalls {
    pub use atlas_define_syscall::definitions::{
        atlas_memcmp_, atlas_memcpy_, atlas_memmove_, atlas_memset_,
    };
}

/// Check that two regions do not overlap.
#[cfg(any(test, not(target_os = "atlas")))]
...
pub mod stubs {
    ...
    /// # Safety
    pub unsafe fn atlas_memcpy(dst: *mut u8, src: *const u8, n: usize) {
    ...
    /// # Safety
    pub unsafe fn atlas_memmove(dst: *mut u8, src: *const u8, n: usize) {
        core::ptr::copy(src, dst, n);
    }
    /// # Safety
    pub unsafe fn atlas_memcmp(s1: *const u8, s2: *const u8, n: usize, result: *mut i32) {
        let mut i = 0;
        while i < n {
            let a = *s1.add(i);
            let b = *s2.add(i);
    ...
    /// # Safety
    pub unsafe fn atlas_memset(s: *mut u8, c: u8, n: usize) {
...
#[inline]
pub unsafe fn atlas_memcpy(dst: &mut [u8], src: &[u8], n: usize) {
    #[cfg(target_os = "atlas")]
    syscalls::atlas_memcpy_(dst.as_mut_ptr(), src.as_ptr(), n as u64);

    #[cfg(not(target_os = "atlas"))]
    stubs::atlas_memcpy(dst.as_mut_ptr(), src.as_ptr(), n);
}
...
pub unsafe fn atlas_memmove(dst: *mut u8, src: *const u8, n: usize) {
    #[cfg(target_os = "atlas")]
    syscalls::atlas_memmove_(dst, src, n as u64);
...
/// likely introduce undefined behavior.
#[inline]
pub unsafe fn atlas_memcmp(s1: &[u8], s2: &[u8], n: usize) -> i32 {
    let mut result: MaybeUninit<i32> = MaybeUninit::uninit();

    #[cfg(target_os = "atlas")]
    syscalls::atlas_memcmp_(s1.as_ptr(), s2.as_ptr(), n as u64, result.as_mut_ptr());

    #[cfg(not(target_os = "atlas"))]
    stubs::atlas_memcmp(s1.as_ptr(), s2.as_ptr(), n, result.as_mut_ptr());
...
pub unsafe fn atlas_memset(s: &mut [u8], c: u8, n: usize) {
    #[cfg(target_os = "atlas")]
    syscalls::atlas_memset_(s.as_mut_ptr(), c, n as u64);

    #[cfg(not(target_os = "atlas"))]
    stubs::atlas_memset(s.as_mut_ptr(), c, n);
}
