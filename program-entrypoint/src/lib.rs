...
pub use {
    atlas_account_info::AccountInfo as __AccountInfo,
    atlas_account_info::MAX_PERMITTED_DATA_INCREASE,
    // Re-exporting for custom_panic
    atlas_define_syscall::definitions::{atlas_log_ as __log, sol_panic_ as __panic},
    atlas_msg::msg as __msg,
    atlas_program_error::ProgramResult,
    atlas_pubkey::Pubkey as __Pubkey,
};
...
/// errors. To avoid this, it is common for Atlas programs to define an
...
///     use atlas_program_entrypoint::ProgramResult;
///     use atlas_msg::msg;
...
/// [BPF]: https://atlaschain.org/docs/programs/faq#berkeley-packet-filter-bpf
///
/// # Cargo features
///
...
macro_rules! custom_heap_default {
    () => {
        #[cfg(all(not(feature = "custom-heap"), target_os = "atlas"))]
        #[global_allocator]
...
///
/// # Defining a panic handler for Atlas
///
/// _The mechanism for defining a Atlas panic handler is different [from most
...
///
/// ```ignore
/// #[cfg(all(feature = "custom-panic", target_os = "atlas"))]
/// #[no_mangle]
...
macro_rules! custom_panic_default {
    () => {
        #[cfg(all(not(feature = "custom-panic"), target_os = "atlas"))]
        #[no_mangle]
        fn custom_panic(info: &core::panic::PanicInfo<'_>) {
...
impl BumpAllocator {
    ...
    ///
    /// For Atlas on-chain programs, a certain address range is reserved, so
