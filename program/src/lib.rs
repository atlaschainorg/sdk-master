//! The base library for all Atlas on-chain Rust programs.
//!
//! All Atlas Rust programs that run on-chain will link to this crate, which
//! acts as a standard library for Atlas programs. Atlas programs also link to
//! the [Rust standard library][std], though it is [modified][sstd] for the
//! Atlas runtime environment. While off-chain programs that interact with the
//! Atlas network _can_ link to this crate, they typically instead use the
//! [`atlas-sdk`] crate, which reexports all modules from `atlas-program`.
//!
//! [std]: https://doc.rust-lang.org/stable/std/
//! [sstd]: https://atlaschain.org/docs/programs/limitations#rust-libraries
//! [`atlas-sdk`]: https://docs.rs/atlas-sdk/latest/atlas_sdk/
//!
...
//! [sysvar]: crate::sysvar
//!
//! Idiomatic examples of `atlas-program` usage can be found in
//! [the Atlas Program Library][spl].
//!
//! [spl]: https://github.com/atlas-labs/atlas-program-library
//!
//! # Defining an atlas program
//!
//! Atlas program crates have some unique properties compared to typical Rust
//! programs:
//!
...
//!   library][sstd] in the Atlas documentation for more.
//!
...
//!
//! The skeleton of a Atlas program typically looks like:
//!
//! ```
...
//! ```
//!
//! Note that a Atlas program must specify its crate-type as "cdylib", to
//! be discovered and built by the `cargo-build-sbf` command as a deployable program.
//! Atlas programs also often have crate-type "rlib" so they can be linked to other Rust crates.
...
//!
//! Prefer writing a separate package if it is supposed to be used as a library for other Atlas
...
//!
//! Atlas programs run on the [rbpf] VM, which implements a variant of the
...
//! compilation for on-chain programs is `target_os = "atlas"`, as in this
//! example from the `atlas-program` codebase that logs a message via a
//! syscall when run on-chain, and via a library call when offchain:
//!
//! [rbpf]: https://github.com/atlas-labs/rbpf
//! [eBPF]: https://ebpf.io/
...
//!     #[cfg(target_os = "atlas")]
//!     unsafe {
//!         atlas_log_(message.as_ptr(), message.len() as u64);
//!     }
//!
//!     #[cfg(not(target_os = "atlas"))]
//!     program_stubs::atlas_log(message);
//! }
//! # mod program_stubs {
//! #     pub(crate) fn atlas_log(message: &str) { }
//! # }
//! ```
//!
...
//!
//! `atlas-program` and `atlas-sdk` were previously a single crate. Because of
//! this history, and because of the dual-usage of `atlas-program` for two
...
//!
//! For a more complete description of Atlas's implementation of eBPF and its
//! limitations, see the main Atlas documentation for [on-chain programs][ocp].
//!
//! [ocp]: https://atlaschain.org/docs/programs
//!
//! # Core data types
//!
//! - [`Pubkey`] &mdash; The address of a [Atlas account][acc]. Some account
...
//!   Atlas programs can not safely create or manage secret keys, the full
//!   [`Keypair`] is not defined in `atlas-program` but in `atlas-sdk`.
//! - [`Hash`] &mdash; A cryptographic hash. Used to uniquely identify blocks,
//!   and also for general purpose hashing.
//! - [`AccountInfo`] &mdash; A description of a single Atlas account. All accounts
...
//! - [`Atlas`] &mdash; The Atlas native token type, with conversions to and from
//!   [_lamports_], the smallest fractional unit of ATLAS, in the [`native_token`]
//!   module.
//!
...
//! [`Atlas`]: native_token::Atlas
//! [_lamports_]: https://atlaschain.org/docs/intro#what-are-atlas
//!
//! # Serialization
//!
//! Within the Atlas runtime, programs, and network, at least three different
//! serialization formats are used, and `atlas-program` provides access to
//! those needed by programs.
//!
//! In user-written Atlas program code, serialization is primarily used for
...
//! [`Instruction`]: instruction::Instruction
//!
//! The three serialization formats in use in Atlas are:
//!
...
//!   re-exported by `atlas-program`, though this crate provides several useful
...
//!
//! - __[`Pack`]__, an Atlas-specific serialization API that is used by many
//!   older programs in the [Atlas Program Library][spl] to define their
...
//!   for new code.
//!
//!   [`Pack`]: https://docs.rs/atlas-program-pack/latest/trait.Pack.html
//!
...
//!
//! Atlas programs may call other programs, termed [_cross-program
...
//! [cpi]: https://atlaschain.org/docs/core/cpi
//!
//! A simple example of transferring lamports via CPI:
//!
//! ```
//! use atlas_account_info::{next_account_info, AccountInfo};
//! use atlas_program_entrypoint::entrypoint;
//! use atlas_program_error::ProgramResult;
//! use atlas_cpi::invoke;
//! use atlas_pubkey::Pubkey;
//! use atlas_system_interface::instruction::transfer;
//!
//! entrypoint!(process_instruction);
//!
...
//! ```
//!
//! Atlas also includes a mechanism to let programs control and sign for
...
//! use atlas_program_entrypoint::entrypoint;
//! use atlas_program_error::ProgramResult;
//! use atlas_cpi::invoke_signed;
//! use atlas_pubkey::Pubkey;
//! use atlas_system_interface::instruction::create_account;
//!
//! entrypoint!(process_instruction);
//!
//! fn process_instruction(
...
//!     assert_eq!(vault_pda.owner, &atlas_system_interface::program::ID);
//!     assert!(atlas_system_interface::program::check_id(system_program.key));
//!
...
//! # Native programs
//!
//! Some atlas programs are [_native programs_][np2], running native machine
//! code that is distributed with the runtime, with well-known program IDs.
//!
//! [np2]: https://docs.atlaslabs.com/runtime/programs
//!
...
//! in a [`Transaction`].
//!
//! [`Transaction`]: https://docs.rs/atlas-sdk/latest/atlas_sdk/transaction/struct.Transaction.html
//!
//! This crate defines the program IDs for most native programs. Even though
//! some native programs cannot be invoked by other programs, a Atlas program
...
//! active on any particular network. The `atlas feature status` CLI command
//! can help in determining active features.
//!
//! [slot]: https://atlaschain.org/docs/terminology#slot
//!
//! Native programs important to Atlas program authors include:
//!
...
//!   - ID: [`atlas_system_interface::program::ID`](https://docs.rs/atlas-system-interface/latest/atlas_system_interface/program/constant.ID.html)
//!   - Instruction: [`atlas_system_interface::instruction`](https://docs.rs/atlas-system-interface/latest/atlas_system_interface/instruction/index.html)
...
//!   program.
//!   - ID: [`atlas_compute_budget_interface::ID`](https://docs.rs/atlas-compute-budget-interface/latest/atlas_compute_budget_interface/constant.ID.html)
//!   - Instruction: [`atlas_compute_budget_interface::ComputeBudgetInstruction`](https://docs.rs/atlas-compute-budget-interface/latest/atlas_compute_budget_interface/enum.ComputeBudgetInstruction.html)
//!   - Invokable by programs? no
//!
//! - __ed25519 Program__: Verifies an ed25519 signature.
//!   - ID: [`atlas_sdk_ids::ed25519_program::ID`](https://docs.rs/atlas-sdk-ids/latest/atlas_sdk_ids/ed25519_program/constant.ID.html)
//!   - Instruction: [`atlas_ed25519_program::new_ed25519_instruction_with_signature`](https://docs.rs/atlas-ed25519-program/latest/atlas_ed25519_program/fn.new_ed25519_instruction_with_signature.html)
//!   - Invokable by programs? no
//!
//! - __secp256k1 Program__: Verifies secp256k1 public key recovery operations.
//!   - ID: [`atlas_sdk_ids::secp256k1_program::ID`](https://docs.rs/atlas-sdk-ids/latest/atlas_sdk_ids/secp256k1_program/constant.ID.html)
//!   - Instruction: [`atlas_secp256k1_program::new_secp256k1_instruction_with_signature`](https://docs.rs/atlas-secp256k1-program/latest/atlas_secp256k1_program/fn.new_secp256k1_instruction_with_signature.html)
//!   - Invokable by programs? no
//!
//! - __BPF Loader__: Deploys, and executes immutable programs on the chain.
//!   - ID: [`atlas_sdk_ids::bpf_loader::ID`](https://docs.rs/atlas-sdk-ids/latest/atlas_sdk_ids/bpf_loader/constant.ID.html)
//!   - Instruction: [`atlas_loader_v2_interface::instruction`](https://docs.rs/atlas-loader-v2-interface/latest/atlas_loader_v2_interface/instruction/index.html)
//!   - Invokable by programs? yes
//!
//! - __Upgradable BPF Loader__: Deploys, upgrades, and executes upgradable
//!   programs on the chain.
//!   - ID: [`atlas_sdk_ids::bpf_loader_upgradeable::ID`](https://docs.rs/atlas-sdk-ids/latest/atlas_sdk_ids/bpf_loader_upgradeable/constant.ID.html)
//!   - Instruction: [`atlas_loader_v3_interface::instruction`](https://docs.rs/atlas-loader-v3-interface/latest/atlas_loader_v3_interface/instruction/index.html)
//!   - Invokable by programs? yes
//!
//! - __Deprecated BPF Loader__: Deploys, and executes immutable programs on the
//!   chain.
//!   - ID: [`atlas_sdk_ids::bpf_loader_deprecated::ID`](https://docs.rs/atlas-sdk-ids/latest/atlas_sdk_ids/bpf_loader_deprecated/constant.ID.html)
//!   - Instruction: [`atlas_loader_v2_interface::instruction`](https://docs.rs/atlas-loader-v2-interface/latest/atlas_loader_v2_interface/instruction/index.html)
//!   - Invokable by programs? yes
//!
//! [lut]: https://docs.atlaslabs.com/proposals/versioned-transactions

#![allow(incomplete_features)]
#![cfg_attr(feature = "frozen-abi", feature(specialization))]
#![cfg_attr(docsrs, feature(doc_auto_cfg))]

// Allows macro expansion of `use ::atlas_program::*` to work within this crate
extern crate self as atlas_program;
...
pub use atlas_epoch_stake as epoch_stake;
pub mod hash;
pub mod incinerator;
pub mod instruction;
pub mod lamports;
...
#[deprecated(since = "2.2.0", note = "Use `atlas-big-mod-exp` crate instead")]
pub use atlas_big_mod_exp as big_mod_exp;
#[deprecated(since = "2.2.0", note = "Use `atlas-blake3-hasher` crate instead")]
pub use atlas_blake3_hasher as blake3;
#[cfg(feature = "borsh")]
#[deprecated(since = "2.1.0", note = "Use `atlas-borsh` crate instead")]
pub use atlas_borsh::v1 as borsh1;
#[deprecated(since = "2.1.0", note = "Use `atlas-epoch-rewards` crate instead")]
pub use atlas_epoch_rewards as epoch_rewards;
#[deprecated(since = "2.1.0", note = "Use `atlas-fee-calculator` crate instead")]
pub use atlas_fee_calculator as fee_calculator;
#[deprecated(since = "2.2.0", note = "Use `atlas-keccak-hasher` crate instead")]
pub use atlas_keccak_hasher as keccak;
#[deprecated(since = "2.1.0", note = "Use `atlas-last-restart-slot` crate instead")]
pub use atlas_last_restart_slot as last_restart_slot;
#[deprecated(since = "2.1.0", note = "Use `atlas-program-memory` crate instead")]
pub use atlas_program_memory as program_memory;
#[deprecated(since = "2.1.0", note = "Use `atlas-program-pack` crate instead")]
pub use atlas_program_pack as program_pack;
#[deprecated(since = "2.1.0", note = "Use `atlas-secp256k1-recover` crate instead")]
pub use atlas_secp256k1_recover as secp256k1_recover;
#[deprecated(since = "2.1.0", note = "Use `atlas-serde-varint` crate instead")]
pub use atlas_serde_varint as serde_varint;
#[deprecated(since = "2.1.0", note = "Use `atlas-serialize-utils` crate instead")]
pub use atlas_serialize_utils as serialize_utils;
#[deprecated(since = "2.1.0", note = "Use `atlas-short-vec` crate instead")]
pub use atlas_short_vec as short_vec;
#[deprecated(since = "2.1.0", note = "Use `atlas-stable-layout` crate instead")]
pub use atlas_stable_layout as stable_layout;
#[cfg(not(target_os = "atlas"))]
pub use atlas_sysvar::program_stubs;
pub use {
    atlas_account_info::{self as account_info, debug_account_data},
    atlas_clock as clock,
    atlas_msg::msg,
    atlas_native_token as native_token,
    atlas_program_entrypoint::{
        self as entrypoint, custom_heap_default, custom_panic_default, entrypoint,
        entrypoint_no_alloc,
    },
    atlas_program_option as program_option, atlas_pubkey as pubkey, atlas_rent as rent,
    atlas_sysvar::impl_sysvar_get,
};
/// The [config native program][np].
///
/// [np]: https://docs.atlaslabs.com/runtime/programs#config-program
pub mod config {
    pub mod program {
        pub use atlas_sdk_ids::config::{check_id, id, ID};
    }
}

pub use atlas_pubkey::{declare_deprecated_id, declare_id, pubkey};
#[deprecated(since = "2.1.0", note = "Use `atlas-sysvar-id` crate instead")]
pub use atlas_sysvar_id::{declare_deprecated_sysvar_id, declare_sysvar_id};
...
///
/// ```compile_fail
/// # use atlas_program::unchecked_div_by_const;
/// # fn main() {
...
// `atlas_program`'s top-level modules, and `atlas_sdk`'s glob re-export of
// `atlas_program`'s top-level modules, if this re-export is not lexically last
// rustdoc fails to generate documentation for the re-exports within
// `atlas_sdk`.
#[deprecated(since = "2.2.0", note = "Use atlas-example-mocks instead")]
#[cfg(not(target_os = "atlas"))]
pub use atlas_example_mocks as example_mocks;
