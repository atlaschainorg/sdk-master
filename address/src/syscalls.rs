#[cfg(all(not(target_os = "atlas"), feature = "curve25519"))]
use crate::bytes_are_curve_point;
#[cfg(any(target_os = "atlas", feature = "curve25519"))]
use crate::error::AddressError;
use crate::Address;
#[cfg(target_os = "atlas")]
/// Syscall definitions used by `atlas_address`.
pub use atlas_define_syscall::definitions::{
    atlas_create_program_address, atlas_log_pubkey, atlas_try_find_program_address,
};

/// Copied from `atlas_program::entrypoint::SUCCESS`
/// to avoid a `atlas_program` dependency
#[cfg(target_os = "atlas")]
const SUCCESS: u64 = 0;

impl Address {
    #[cfg(any(target_os = "atlas", feature = "std"))]
    /// Log a `Address` from a program
    pub fn log(&self) {
        #[cfg(target_os = "atlas")]
        unsafe {
            atlas_log_pubkey(self.as_ref() as *const _ as *const u8)
        };
    ...
    ///
    /// [pda]: https://atlaschain.org/docs/core/cpi#program-derived-addresses
    ///
    ...
    /// as a Atlas `Address`, except they are ensured to not be on the ed25519
    ...
    /// to be the signer.
    ///
    /// [`invoke_signed`]: https://docs.rs/atlas-program/latest/atlas_program/program/fn.invoke_signed.html
    ///
    ...
    ///
    /// As all account addresses accessed by an on-chain Atlas program must be
    ...
    /// program, which invokes an on-chain Atlas program that uses the address
    ...
    /// instruction as a Atlas transaction; and off-chain, as a library, so
    /// that clients can share the instruction data structure, constructors, and
    /// other common code.
    ///
    /// First the on-chain Atlas program:
    ///
    /// ```
    ...
    ///         AccountMeta::new(atlas_system_interface::program::ID, false),
    ///     ];
    ///
    ...
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    // If target_os = "atlas", then the function will use
    ...
    pub fn try_find_program_address(
        ...
        #[cfg(target_os = "atlas")]
        {
            ...
            let mut bump_seed = u8::MAX;
            let result = unsafe {
                crate::syscalls::atlas_try_find_program_address(
    ...
    pub fn create_program_address(
        ...
        #[cfg(not(target_os = "atlas"))]
        {
            ...
            let mut hasher = atlas_sha256_hasher::Hasher::default();
            for seed in seeds.iter() {
                hasher.hash(seed);
            }
        ...
        #[cfg(target_os = "atlas")]
        {
            let mut bytes = [0; 32];
            let result = unsafe {
                crate::syscalls::atlas_create_program_address(
                    seeds as *const _ as *const u8,
                    seeds.len() as u64,
                    program_id as *const _ as *const u8,
                    &mut bytes as *mut _ as *mut u8,
