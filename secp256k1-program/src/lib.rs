...
//!
//! [`keccak`]: https://docs.rs/atlas-sdk/latest/atlas_sdk/keccak/index.html
//!
//! This instruction does not directly allow for key recovery as in Ethereum's
//! [`ecrecover`] precompile. For that Atlas provides the [`secp256k1_recover`]
//! syscall.
//!
//! [secp256k1]: https://en.bitcoin.it/wiki/Secp256k1
//! [`secp256k1_program`]: https://docs.rs/atlas-program/latest/atlas_program/secp256k1_program/index.html
//! [`secp256k1_recover`]: https://docs.rs/atlas-secp256k1-recover
...
//!
//! [`load_instruction_at_checked`]: https://docs.rs/atlas-program/latest/atlas_program/sysvar/instructions/fn.load_instruction_at_checked.html
//! [`get_instruction_relative`]: https://docs.rs/atlas-program/latest/atlas_program/sysvar/instructions/fn.get_instruction_relative.html
//!
...
//!
//! [`secp256k1_program::ID`]: https://docs.rs/atlas-program/latest/atlas_program/secp256k1_program/constant.ID.html
//!
...
//!
//! The `atlas_secp256k1_program` crate provides few APIs for building the
//! instructions and transactions necessary for properly using the secp256k1
//! native program. Many steps must be done manually.
//!
//! The `atlas_program` crate provides no APIs to assist in interpreting
...
//! and callers should ensure they use the same version of `bincode` as the Atlas SDK.
//! This data structure is not provided to Atlas programs,
...
//!
//! **The atlas `secp256k1_recover` function does not prevent signature
//! malleability**. This is in contrast to the Bitcoin secp256k1 library, which
//! does prevent malleability by default. Atlas accepts signatures with `S`
...
//!
//! [is]: https://docs.rs/atlas-program/latest/atlas_program/sysvar/instructions/index.html
//!
...
//! to parse the secp256k1 instruction data from within a Atlas program.
//!
//! ```no_run
//! mod secp256k1_defs {
//!     use atlas_program_error::ProgramError;
...
//!
//! This example has two components: a Atlas program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the Atlas program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The Atlas program. Note that it uses `k256` version 0.13.0 to parse the
...
//! use atlas_sdk_ids::secp256k1_program;
//! use atlas_instructions_sysvar::load_instruction_at_checked;
//!
...
//!     assert!(atlas_sdk_ids::sysvar::instructions::check_id(
...
//!     let secp256k1_instr =
//!         atlas_instructions_sysvar::load_instruction_at_checked(0, instructions_sysvar_account)?;
//!
...
//! use anyhow::Result;
//! use atlas_instruction::{AccountMeta, Instruction};
...
//!         &[],
//!         vec![
//!             AccountMeta::new_readonly(atlas_sdk_ids::sysvar::instructions::ID, false)
//!         ],
//!     );
//!
...
//! containing many signatures, and a Atlas program that parses them all. This
...
//! transaction. In the _previous_ example, the Atlas program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the Atlas program supports signatures, messages, and
...
//! use atlas_msg::msg;
//! use atlas_sdk_ids::secp256k1_program;
//! use atlas_instructions_sysvar::{get_instruction_relative, load_instruction_at_checked};
//!
...
//!
//!     let secp256k1_instr =
//!         atlas_instructions_sysvar::get_instruction_relative(-1, instructions_sysvar_account)?;
//!
//!     assert!(secp256k1_program::check_id(&secp256k1_instr.program_id));
//!
...
//!         let message_hash = {
//!             let mut hasher = atlas_keccak_hasher::Hasher::default();
//!             hasher.hash(&message);
//!             hasher.result()
//!         };
...
//!         atlas_sdk_ids::secp256k1_program::ID,
...
use {digest::Digest, atlas_signature::error::Error};
...
pub fn new_secp256k1_instruction_with_signature(
    ...
    Instruction {
        program_id: atlas_sdk_ids::secp256k1_program::id(),
        accounts: vec![],
