...
use {
    ...
    atlas_instruction::{error::InstructionError, AccountMeta, Instruction},
    atlas_pubkey::Pubkey,
    atlas_sdk_ids::{bpf_loader_upgradeable::id, loader_v4, sysvar},
...
pub enum UpgradeableLoaderInstruction {
    ...
    /// # use atlas_sdk_ids::bpf_loader_upgradeable;
    /// # let program_address = &[];
    ...
    ///   4. `[]` Rent sysvar.
    ///   5. `[]` Clock sysvar.
    ///   6. `[]` System program (`atlas_sdk_ids::system_program::id()`).
    ...
    ///   2. `[]` System program (`atlas_sdk::system_program::id()`), optional, used to transfer
    ...
    ///   3. `[]` System program (`atlas_sdk::system_program::id()`), optional, used to transfer
    ///      lamports from the payer to the ProgramData account.
    ...
    Ok(vec![
        ...
        Instruction::new_with_bincode(
            ...
            vec![
                ...
                AccountMeta::new_readonly(atlas_sdk_ids::system_program::id(), false),
...
pub fn extend_program(
    ...
    if let Some(payer_address) = payer_address {
        metas.push(AccountMeta::new_readonly(
            atlas_sdk_ids::system_program::id(),
            false,
        ));
