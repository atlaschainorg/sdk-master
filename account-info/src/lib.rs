...
use {
    ...
    atlas_program_memory::atlas_memset,
    atlas_pubkey::Pubkey,
    std::{
...
impl<'a> AccountInfo<'a> {
    ...
    /// `atlas_program::entrypoint::MAX_PERMITTED_DATA_INCREASE` bytes.
    ///
    /// # Safety
    ///
    ...
    pub fn resize(&self, new_len: usize) -> Result<(), ProgramError> {
        ...
        if len_increase > 0 {
            unsafe { atlas_memset(&mut data[old_len..], 0, len_increase) };
        }
...
/// ```
/// use atlas_program_error::ProgramResult;
/// use atlas_account_info::{AccountInfo, next_account_info, next_account_infos};
/// use atlas_pubkey::Pubkey;
/// # use atlas_program_error::ProgramError;
///
/// pub fn process_instruction(
