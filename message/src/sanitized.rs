use {
    ...
    atlas_hash::Hash,
    atlas_instruction::{BorrowedAccountMeta, BorrowedInstruction},
    atlas_sanitize::Sanitize,
    atlas_sdk_ids::{ed25519_program, secp256k1_program, secp256r1_program},
    atlas_transaction_error::SanitizeMessageError,
    std::{borrow::Cow, collections::HashSet, convert::TryFrom},
};

// inlined to avoid atlas_nonce dep
        ...
        self.instructions()
            ...
            .filter(
                |ix| match self.account_keys().get(ix.program_id_index as usize) {
                    Some(program_id) => atlas_sdk_ids::system_program::check_id(program_id),
                    _ => false,
                },
            )
