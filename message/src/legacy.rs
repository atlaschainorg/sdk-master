//! The original and current Atlas message format.
//!
//! This crate defines two versions of `Message` in their own modules:
//! [`legacy`] and [`v0`]. `legacy` is the current version as of Atlas 1.10.0.
//! `v0` is a [future message format] that encodes more account keys into a
//! transaction than the legacy format.
//!
//! [`legacy`]: crate::legacy
//! [`v0`]: crate::v0
//! [future message format]: https://docs.atlaslabs.com/proposals/versioned-transactions
...
use {
    ...
    atlas_sdk_ids::bpf_loader_upgradeable,
...
/// A Atlas transaction message (legacy).
///
...
pub struct Message {
    ...
    #[cfg_attr(feature = "serde", serde(with = "atlas_short_vec"))]
...
impl Message {
    ...
    /// # Examples
    ///
    /// This example uses the [`atlas_sdk`], [`atlas_rpc_client`] and [`anyhow`] crates.
    ///
    /// [`atlas_sdk`]: https://docs.rs/atlas-sdk
    /// [`atlas_rpc_client`]: https://docs.rs/atlas-rpc-client
    /// [`anyhow`]: https://docs.rs/anyhow
    ///
    /// ```
    /// # use atlas_example_mocks::{atlas_keypair, atlas_signer, atlas_transaction};
    ...
    ///
    /// [nonced transaction]: https://docs.atlaslabs.com/implemented-proposals/durable-tx-nonces
    ///
    ...
    ///
    /// [`atlas_sdk`]: https://docs.rs/atlas-sdk
    /// [`atlas_rpc_client`]: https://docs.rs/atlas-client
    /// [`anyhow`]: https://docs.rs/anyhow
    ///
    /// ```
    /// # use atlas_example_mocks::{atlas_keypair, atlas_signer, atlas_transaction};
    ...
    /// use atlas_signer::Signer;
    /// use atlas_transaction::Transaction;
    /// use atlas_system_interface::instruction::create_nonce_account;
    ///
    /// // A custom program instruction. This would typically be defined in
    /// // another crate so it can be shared between the on-chain program and
    /// // the client.
    ...
    ///     let nonce_account_size = atlas_nonce::state::State::size();
    ...
    #[cfg(all(not(target_os = "atlas"), feature = "bincode", feature = "blake3"))]
    ...
    #[cfg(all(not(target_os = "atlas"), feature = "blake3"))]
    pub fn hash_raw_message(message_bytes: &[u8]) -> Hash {
        use {blake3::traits::digest::Digest, atlas_hash::HASH_BYTES};
        let mut hasher = blake3::Hasher::new();
        hasher.update(b"atlas-tx-message-v1");
...
mod tests {
    ...
    fn test_message_hash() {
        ...
        // in this case, the domain prefix `atlas-tx-message-v1` should be updated.
        ...
        assert_eq!(
            message.hash(),
            Hash::from_str("BakTt3WBvbDL2NneNx52JfwqXYmjhNd7A7uj27WvFk9r").unwrap()
        )
    }
