...
#[cfg(test)]
static_assertions::const_assert_eq!(OffchainMessage::HEADER_LEN, 16);
#[cfg(test)]
static_assertions::const_assert_eq!(v0::OffchainMessage::MAX_LEN, 65516);
#[cfg(test)]
static_assertions::const_assert_eq!(v0::OffchainMessage::MAX_LEN_LEDGER, 1213);
...
pub mod v0 {
    use {
        ...
        atlas_hash::Hash,
        atlas_packet::PACKET_DATA_SIZE,
        atlas_sanitize::SanitizeError,
        atlas_sha256_hasher::Hasher,
    };
...
impl OffchainMessage {
    pub const SIGNING_DOMAIN: &'static [u8] = b"\xffatlas offchain";
...
#[cfg(test)]
mod tests {
    use {super::*, atlas_keypair::Keypair, std::str::FromStr};
    ...
    fn test_offchain_message_ascii() {
        ...
        let serialized = [
            255, 97, 116, 108, 97, 115, 32, 111, 102, 102, 99, 104, 97, 105, 110, 0, 0, 12, 0,
            84, 101, 115, 116, 32, 77, 101, 115, 115, 97, 103, 101,
        ];
        let hash = Hash::from_str("FeZAMvAtEzTL2aFeS4cmB7WKqo8kD8ekKSfgeWpDvTPA").unwrap();
    ...
    fn test_offchain_message_utf8() {
        ...
        let serialized = [
            255, 97, 116, 108, 97, 115, 32, 111, 102, 102, 99, 104, 97, 105, 110, 0, 1, 35, 0,
        ...
        let hash = Hash::from_str("7eUMgsKNb6zPCJxXdgY2MB5wMQFiouvDp3xc9p28b2sA").unwrap();
        assert_eq!(message.serialize().unwrap(), serialized);
        assert_eq!(message.hash().unwrap(), hash);
        assert_eq!(message, OffchainMessage::deserialize(&serialized).unwrap());
    }
