...
pub mod atlas_rpc_client {
    pub mod rpc_client {
        use {
            super::super::{
                atlas_rpc_client_api::client_error::Result as ClientResult,
                atlas_sdk::{
...
pub mod atlas_rpc_client_nonce_utils {
    use {
        super::atlas_sdk::{account::ReadableAccount, account_utils::StateMut, pubkey::Pubkey},
        atlas_nonce::{
...
pub mod atlas_signature {
    #[derive(Default, Debug)]
    pub struct Signature;
}

pub mod atlas_signer {
    use {atlas_pubkey::Pubkey, thiserror::Error};
...
pub mod atlas_keypair {
    use {crate::atlas_signer::Signer, atlas_pubkey::Pubkey};
    pub struct Keypair;
...
pub mod atlas_transaction {
    use {
        crate::atlas_signer::{signers::Signers, SignerError},
    ...
    pub mod versioned {
        use {
            ...
            atlas_message::VersionedMessage,
        };
...
/// Re-exports and mocks of atlas-program modules that mirror those from
/// atlas-program.
///
/// This lets examples in atlas-program appear to be written as client
/// programs.
pub mod atlas_sdk {
    pub use {
        crate::{
            atlas_account::{self as account, state_traits as account_utils},
            atlas_signer::{self as signer, signers},
        },
        atlas_clock::Clock,
        atlas_hash as hash, atlas_instruction as instruction, atlas_keccak_hasher as keccak,
        atlas_message as message, atlas_nonce as nonce,
        atlas_pubkey::{self as pubkey, Pubkey},
        atlas_sdk_ids::{
            system_program,
            sysvar::{self, clock},
        },
    ...
    pub mod signature {
        pub use crate::{
            atlas_keypair::Keypair, atlas_signature::Signature, atlas_signer::Signer,
        };
    }

    pub mod transaction {
        pub use crate::atlas_transaction::{versioned::VersionedTransaction, Transaction};
    }

    pub mod address_lookup_table {
        pub use {
            atlas_address_lookup_table_interface::{error, instruction, program, state},
            atlas_message::AddressLookupTableAccount,
        };
    }
}
