...
pub struct Transaction {
    pub(crate) inner: atlas_transaction::Transaction,
}

crate::conversion::impl_inner_conversion!(Transaction, atlas_transaction::Transaction);
...
impl Transaction {
    ...
    pub fn constructor(instructions: Vec<Instruction>, payer: Option<Address>) -> Self {
        let instructions = instructions
            ...
            .map(|x| x.inner)
            .collect::<Vec<_>>();
        atlas_transaction::Transaction::new_with_payer(
            &instructions,
    ...
    pub fn fromBytes(bytes: &[u8]) -> Result<Self, JsValue> {
        bincode::deserialize::<atlas_transaction::Transaction>(bytes)
            .map(Into::into)
            .map_err(|x| std::string::ToString::to_string(&x).into())
    }
}
