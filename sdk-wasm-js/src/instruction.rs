...
pub struct Instruction {
    pub(crate) inner: atlas_instruction::Instruction,
}

crate::conversion::impl_inner_conversion!(Instruction, atlas_instruction::Instruction);
...
impl Instruction {
    ...
    pub fn constructor(program_id: Address) -> Self {
        atlas_instruction::Instruction::new_with_bytes(program_id.inner, &[], std::vec::Vec::new())
            .into()
    }
...
pub struct AccountMeta {
    pub(crate) inner: atlas_instruction::AccountMeta,
}

crate::conversion::impl_inner_conversion!(AccountMeta, atlas_instruction::AccountMeta);

#[wasm_bindgen]
impl AccountMeta {
    /// Create a new writable `AccountMeta`
    pub fn newWritable(address: Address, is_signer: bool) -> Self {
        atlas_instruction::AccountMeta::new(address.inner, is_signer).into()
    }

    /// Create a new readonly `AccountMeta`
    pub fn newReadonly(address: Address, is_signer: bool) -> Self {
        atlas_instruction::AccountMeta::new_readonly(address.inner, is_signer).into()
    }
}
