...
pub struct Address {
    pub(crate) inner: atlas_address::Address,
}

crate::conversion::impl_inner_conversion!(Address, atlas_address::Address);
...
impl Address {
    ...
    pub fn constructor(value: JsValue) -> Result<Self, JsValue> {
        if let Some(base58_str) = value.as_string() {
            ...
            atlas_address::Address::try_from(bytes)
                .map(Into::into)
                .map_err(|err| JsValue::from(std::format!("Invalid Array address: {err:?}")))
        } else if value.is_undefined() {
            Ok(atlas_address::Address::default().into())
        } else {
    ...
    pub fn createWithSeed(base: &Self, seed: &str, owner: &Self) -> Result<Self, JsValue> {
        atlas_address::Address::create_with_seed(&base.inner, seed, &owner.inner)
            .map(Into::into)
            .map_err(display_to_jsvalue)
    }
    ...
    pub fn createProgramAddress(
        ...
        atlas_address::Address::create_program_address(seeds_slice.as_slice(), &program_id.inner)
            .map(Into::into)
            .map_err(display_to_jsvalue)
    }
    ...
    pub fn findProgramAddress(
        ...
        let (address, bump_seed) = atlas_address::Address::find_program_address(
