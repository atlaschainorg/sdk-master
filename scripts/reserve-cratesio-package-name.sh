...
FLAGS:
        ...
        --no-prefix           Do not require \`agave-\` or \`atlas-\` prefix on PACKAGE_NAME
...
if ${require_prefix} && ! [[ "${package_name}" =~ ^(agave|atlas)- ]]; then
  # shellcheck disable=SC2016 # backticks are not a command here
  echo 'error: PACKAGE_NAME MUST start with `agave-` or `atlas-`' 1>&2
  display_help
  exit 1
fi
