...
#   $ ./scripts/test-coverage.sh -p atlas-pubkey
#   $ ./scripts/test-coverage.sh -p atlas-pubkey -p atlas-sdk-ids [-p ...]
#
# Custom folder name. (default: $(git rev-parse --short=9 HEAD))
#   $ COMMIT_HASH=xxx ./script/coverage.sh -p atlas-account-decoder
...
# Most verbose log level (trace) is enabled for all atlas code to make log!
# macro code green always. Also, forcibly discard the vast amount of log by
# redirecting the stderr.
RUST_LOG="atlas=trace,$RUST_LOG" \
