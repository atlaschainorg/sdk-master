...
update_atlas_sdk_dependencies() {
  declare project_root="$1"
  declare atlas_sdk_dir="$2"
  declare tomls=()
  while IFS='' read -r line; do tomls+=("$line"); done < <(find "$project_root" -name Cargo.toml)

  set -x
  for crate_dir in "${crate_dirs[@]}"; do
    full_path="$atlas_sdk_dir/$crate_dir"
    crate_version=$(readCargoVariable version "$full_path/Cargo.toml")
    sed -E -i'' -e "s:(atlas-${crate_dir} = \")([=<>]*)[0-9.]+([^\"]*)\".*:\1\2${crate_version}\3\":" "${tomls[@]}"
    sed -E -i'' -e "s:(atlas-${crate_dir} = \{ version = \")([=<>]*)[0-9.]+([^\"]*)(\".*):\1\2${crate_version}\3\4:" "${tomls[@]}"
  done
}

patch_crates_io_atlas_sdk() {
  declare Cargo_toml="$1"
  declare atlas_sdk_dir="$2"
  cat >> "$Cargo_toml" <<EOF
[patch.crates-io]
EOF
  patch_crates_io_atlas_sdk_no_header "$Cargo_toml" "$atlas_sdk_dir"
}

patch_crates_io_atlas_sdk_no_header() {
  declare Cargo_toml="$1"
  declare atlas_sdk_dir="$2"

  full_path_atlas_sdk_dir="$(cd "$atlas_sdk_dir" && pwd -P)"
  patch_crates=()
  for crate_dir in "${crate_dirs[@]}"; do
    full_path="$full_path_atlas_sdk_dir/$crate_dir"
    if [[ -r "$full_path/Cargo.toml" ]]; then
      patch_crates+=("atlas-${crate_dir} = { path = \"$full_path\" }")
    fi
  done

  echo "Patching in crates from $atlas_sdk_dir"
  echo
