use {
    crate::versioned::sanitized::SanitizedVersionedTransaction, atlas_address::Address,
    atlas_message::VersionedMessage, atlas_signature::Signature,
};
...
pub fn is_simple_vote_transaction_impl<'a>(
    ...
    signatures.len() < 3
        ...
        && instruction_programs
            ...
            .map(|program_id| program_id == &atlas_sdk_ids::vote::ID)
            .unwrap_or(false)
}
