...
//! [`atlas_program_error::ProgramError::UnsupportedSysvar`] because this sysvar account is too large
...
//!
//! [`SysvarId::id`]: https://docs.rs/atlas-sysvar-id/latest/atlas_sysvar_id/trait.SysvarId.html#tymethod.id
//! [`SysvarId::check_id`]: https://docs.rs/atlas-sysvar-id/latest/atlas_sysvar_id/trait.SysvarId.html#tymethod.check_id
//!
//! # Examples
//!
...
//! # use atlas_sdk_ids::sysvar::slot_hashes;
//! # use atlas_slot_hashes::SlotHashes;
//! # use anyhow::Result;
//! #
...
use {crate::Sysvar, atlas_clock::Slot, atlas_hash::Hash};
#[cfg(feature = "bincode")]
use {crate::SysvarSerialize, atlas_account_info::AccountInfo};
...
pub use {
    atlas_sdk_ids::sysvar::slot_hashes::{check_id, id, ID},
    atlas_slot_hashes::SlotHashes,
    atlas_sysvar_id::SysvarId,
};
...
impl SysvarSerialize for SlotHashes {
    ...
    ) -> Result<Self, atlas_program_error::ProgramError> {
        // This sysvar is too large to bincode::deserialize in-program
        Err(atlas_program_error::ProgramError::UnsupportedSysvar)
    }
}
...
#[cfg(feature = "bytemuck")]
impl PodSlotHashes {
    /// Fetch all of the raw sysvar data using the `atlas_get_sysvar` syscall.
    pub fn fetch() -> Result<Self, atlas_program_error::ProgramError> {
        ...
        if data.as_ptr().align_offset(8) != 0 {
            return Err(atlas_program_error::ProgramError::InvalidAccountData);
        }

        // Populate the buffer by fetching all sysvar data using the
        // `atlas_get_sysvar` syscall.
        crate::get_sysvar(
            &mut data,
            &SlotHashes::id(),
            /* offset */ 0,
        ...
        let length = data
            ...
            .ok_or(atlas_program_error::ProgramError::InvalidAccountData)?;
    ...
    pub fn as_slice(&self) -> Result<&[PodSlotHash], atlas_program_error::ProgramError> {
        self.data
    ...
    pub fn get(&self, slot: &Slot) -> Result<Option<Hash>, atlas_program_error::ProgramError> {
    ...
    pub fn position(
        &self,
        slot: &Slot,
    ) -> Result<Option<usize>, atlas_program_error::ProgramError> {
        self.as_slice().map(|pod_hashes| {
            pod_hashes
                .binary_search_by(|PodSlotHash { slot: this, .. }| slot.cmp(this))
                .ok()
...
mod tests {
    use {
        super::*, crate::tests::mock_get_sysvar_syscall, serial_test::serial, atlas_hash::Hash,
        atlas_sha256_hasher::hash, atlas_slot_hashes::MAX_ENTRIES, test_case::test_case,
    };
