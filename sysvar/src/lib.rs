...
//!     instruction_data: &[u8],
//! ) -> ProgramResult {
//!     let clock = atlas_clock::Clock::get()?;
//!     msg!("clock: {:#?}", clock);
//!     Ok(())
//! }
//! ```
//!
//! Since Atlas sysvars are accounts, if the `AccountInfo` is provided to the
...
//!     let clock = atlas_clock::Clock::from_account_info(&clock_account)?;
//!     msg!("clock: {:#?}", clock);
//!     Ok(())
//! }
//! ```
...
//!
//! [`sysvar::ID`]: https://docs.rs/atlas-sdk-ids/latest/atlas_sdk_ids/sysvar/constant.ID.html
//!
...
pub mod __private {
    ...
    pub use {atlas_program_entrypoint::SUCCESS, atlas_program_error::ProgramError};
}
#[cfg(feature = "bincode")]
use {atlas_account_info::AccountInfo, atlas_sysvar_id::SysvarId};
use {atlas_program_error::ProgramError, atlas_pubkey::Pubkey};
...
/// the sysvar data.
//
// Defined in the bpf loader as [`OFFSET_LENGTH_EXCEEDS_SYSVAR`](https://github.com/atlaschainorg/agave/blob/master/programs/bpf_loader/src/syscalls/sysvar.rs#L172).
const OFFSET_LENGTH_EXCEEDS_SYSVAR: u64 = 1;

/// Return value indicating that the sysvar was not found.
//
// Defined in the bpf loader as [`SYSVAR_NOT_FOUND`](https://github.com/atlaschainorg/agave/blob/master/programs/bpf_loader/src/syscalls/sysvar.rs#L171).
...
/// Handler for retrieving a slice of sysvar data from the `atlas_get_sysvar`
/// syscall.
pub fn get_sysvar(
    dst: &mut [u8],
    sysvar_id: &Pubkey,
    offset: u64,
    length: u64,
) -> Result<(), atlas_program_error::ProgramError> {
    // Check that the provided destination buffer is large enough to hold the
    // requested data.
    if dst.len() < length as usize {
        return Err(atlas_program_error::ProgramError::InvalidArgument);
    }
    ...
    #[cfg(target_os = "atlas")]
    let result = unsafe {
        atlas_define_syscall::definitions::atlas_get_sysvar(sysvar_id, var_addr, offset, length)
    };

    #[cfg(not(target_os = "atlas"))]
    let result = crate::program_stubs::atlas_get_sysvar(sysvar_id, var_addr, offset, length);

    match result {
        atlas_program_entrypoint::SUCCESS => Ok(()),
        OFFSET_LENGTH_EXCEEDS_SYSVAR => Err(atlas_program_error::ProgramError::InvalidArgument),
        SYSVAR_NOT_FOUND => Err(atlas_program_error::ProgramError::UnsupportedSysvar),
        // Unexpected errors are folded into `UnsupportedSysvar`.
        _ => Err(atlas_program_error::ProgramError::UnsupportedSysvar),
    }
}
...
mod tests {
    use {
        ...
        atlas_program_entrypoint::SUCCESS,
        atlas_program_error::ProgramError,
        atlas_pubkey::Pubkey,
    ...
    atlas_pubkey::declare_id!("TestSysvar111111111111111111111111111111111");
    impl atlas_sysvar_id::SysvarId for TestSysvar {
        fn id() -> atlas_pubkey::Pubkey {
            id()
        }

        fn check_id(pubkey: &atlas_pubkey::Pubkey) -> bool {
            check_id(pubkey)
        }
    }
