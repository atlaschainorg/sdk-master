...
#[cfg(feature = "atlas-signer-derive")]
...
impl Keypair {
    ...
    /// Derive a `BlsSecretKey` from a Atlas signer
    #[cfg(feature = "atlas-signer-derive")]
...
mod tests {
    ...
    fn test_keygen_derive_from_signer() {
        let atlas_keypair = atlas_keypair::Keypair::new();
        let secret = SecretKey::derive_from_signer(&atlas_keypair, b"alpenglow-vote").unwrap();
        let public: Pubkey = PubkeyProjective::from_secret(&secret).into();
        let keypair = Keypair::derive_from_signer(&atlas_keypair, b"alpenglow-vote").unwrap();
